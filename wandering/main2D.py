...
class Visualisation(gbl.GraphBase): #Класс окна визуализации
    def __init__(self, nomber, xmin = -1, xmax = 1, ymin = -1, ymax = 1, title = '', xlabel = '', ylabel = '', r=3):
        ...
        self.gausX = np.linspace(self.xmin, self.xmax, 2000) #Точки на оси х
    ...
    def animate(self, i, calsFunc, argsFunc): #Функция, рисующая визуализацию
        ...
        for j in range(0, self.hist_count): #Пробегаем по всем кусочкам гистограммы
            x = self.hist_unit*j #Начальная координата полки
            count = 0 #Счётчик частиц
            for prt in particles: #Назодим частицы в диапазоне кусочка
                if prt.rv > x and prt.rv < x+self.hist_unit:
                    count+=1 #считаем их
            self.hist_x.append(x + self.xmin) #Добавляем кооржинаты полки
            self.hist_x.append(x+self.hist_unit + self.xmin)
            self.hist_y.append(count/5 + self.ymin)
            self.hist_y.append(count/5 + self.ymin)
        ...
        #Гаусс
        #Рассчёт по формуле
        self.gausY = 1/np.sqrt(2*np.pi*(i+1)*particles[0].vx**2)*np.exp(-1*((self.gausX-self.xmin)*50 - (i+1)*particles[0].vx*np.sqrt(2))**2/(2*(i+1)*particles[0].vx*np.sqrt(2)))*30+self.ymin
...
class Particle(object): #Класс частицы
    ...
    def moving(self): #Функция, перемещающая частицу
        ...
        self.rv = np.sqrt(self.x+self.y) #радиус вектор
...
class Surface(object): #Объявление класса поверхности
    def __init__(self, title = ''):
        ...
        self.averageR, self.averageRR = 0, 0 #Среднее и среднеквадратичное значнение радиуса
        ...
        plt.gcf().canvas.set_window_title('Задание 3.2 — двумерные блуждания') #Заголовок окна

        self.particles = [] #Кортеж частиц

        for i in range(0, self.pNomber): #Создание частиц
            x = 0 
            y = 0 #для равномерного распределение около нуля
            prt = Particle(x, y, vx=0.1, vy=0.1) #Создаём экземпляр частицы
    ...
    def nextFrameCalc(self, i): #Вычисление параметров системы
        ...
        self.averageR = 0 #Обнуление показателей
        self.averageRR = 0
        for prt in self.particles: #Считыаем новые
            self.averageR += np.sqrt(prt.x**2+prt.y**2)
            self.averageRR += prt.x**2 + prt.y**2
        self.averageR = self.averageR/self.pNomber 
        self.averageRR = self.averageRR/self.pNomber       
    ...
    def averageR_args(self): #Для средней координаты
        return self.t, self.averageR

    def averageRR_args(self): #Для среднеквадратичной координаты
        return self.t, self.averageRR
    ...
    def startDraw(self): #Создание экземпляров графических областей и запуск рисования в них
        ...
        self.visualisation = Visualisation(self.pNomber, xmin=-limit, xmax=limit, ymin=-limit, ymax=limit, r = 3,
        ...
        self.averageRPlot = gbl.Plot(xmin=0, xmax=30, ymin=0, ymax=10, r = 3,
            title='Среднее смещение', xlabel='время t', ylabel='<r(t)>') 
        FuncAnimation(self.fig, self.averageRPlot.animate, fargs=(self.averageR_args, ),
            frames=3000, interval=self.dt*1000, blit=True) 
        
        #Среднеквадратичная координата
        self.fig_ax_3 = self.fig.add_subplot(self.grs[1, 2])
        self.averageRRPlot = gbl.Plot(xmin=0, xmax=30, ymin=0, ymax=10, r = 3,
            title='Среднеквадратичное смещение', xlabel='время t', ylabel='<r^2(t)>') 
        FuncAnimation(self.fig, self.averageRRPlot.animate, fargs=(self.averageRR_args, ),
            frames=3000, interval=self.dt*1000, blit=True)
